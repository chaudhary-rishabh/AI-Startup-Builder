...
describe('admin routes (unit)', () => {
  ...
  it('GET /rag/admin/pinecone-stats without namespace', async () => {
    const res = await app.request('http://localhost/rag/admin/pinecone-stats', {
      headers: { Authorization: `Bearer ${token}` },
    })
    expect(res.status).toBe(200)
  })

  it('GET /rag/admin/pinecone-stats with namespace', async () => {
    const res = await app.request('http://localhost/rag/admin/pinecone-stats?namespace=user_x', {
      headers: { Authorization: `Bearer ${token}` },
    })
    expect(res.status).toBe(200)
    expect(pine.getNamespaceStats).toHaveBeenCalledWith('user_x')
  })
})
