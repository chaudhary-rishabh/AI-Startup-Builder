...
import { getAgentPerformance } from '../services/agentPerformance.service.js'
...
async function rateLimitOk(userId: string): Promise<boolean> {
  ...
  const key = `analytics:rl:agent-perf:${userId}`
  const n = await redis.incr(key)
  if (n === 1) await redis.expire(key, 60)
  return n <= 30
}

routes.get('/agent-performance', zValidator('query', querySchema), async (c) => {
  ...
  return ok(c, await getAgentPerformance(range))
})
