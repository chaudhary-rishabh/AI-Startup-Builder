...
import { getTokenUsageAnalytics } from '../services/tokenUsageAnalytics.service.js'
...
async function rateLimitOk(userId: string): Promise<boolean> {
  ...
  const key = `analytics:rl:token-usage:${userId}`
  const n = await redis.incr(key)
  if (n === 1) await redis.expire(key, 60)
  return n <= 30
}

routes.get('/token-usage', zValidator('query', querySchema), async (c) => {
  ...
  return ok(c, await getTokenUsageAnalytics(range))
})
